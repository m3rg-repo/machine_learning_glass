...
def load_explainer(path='hardness_explainer.pkl'):
    #loading explainer 
    with open(path,'rb') as f:
        explainer = pickle.load(f)
    return explainer

def load_shap_values(path='hardness_shap_values.pkl'):
    #loading shap values
    with open(path,'rb') as f:
        shap_values = pickle.load(f)
    return shap_values

def load_values(path='values.pkl'):
    #loading values
    with open(path,'rb') as f:
        values = pickle.load(f)
    return values

def load_featuers(path='features.pkl'):
